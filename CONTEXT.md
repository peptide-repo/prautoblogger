...
| **quorum** | Minimum number of research agents that must return usable results for the fan-out to proceed. Formula: ⌈N/2⌉+1 (one strict majority). Example: N=3 → quorum=3; N=5 → quorum=4. If fewer than quorum agents return valid results, `Research_Fanout::dispatch()` returns an empty array and the pipeline holds the run (status `held-quorum`). v0.32.2: quorum-miss hold no longer calls Publisher — only a `run_decisions` row is written (no draft post created; Publisher would throw on empty content). |
